#!/usr/bin/env python3
"""Copy reusable local setup from a V2.4.4 project into V2.4.5."""
...
def main() -> None:
    ...
    parser.add_argument("v244_folder", help="Path to the existing Science Diagnostic System V2.4.4 folder")
    args = parser.parse_args()

    old = Path(args.v244_folder).expanduser().resolve()
    new = Path(__file__).resolve().parents[1]
    if not old.exists():
        raise SystemExit(f"V2.4.4 folder not found: {old}")
