#!/usr/bin/env python3
"""Copy reusable local setup from a V2.4.3 project into V2.4.4."""
...
def main() -> None:
    ...
    parser.add_argument("v243_folder", help="Path to the existing Science Diagnostic System V2.4.3 folder")
    args = parser.parse_args()

    old = Path(args.v243_folder).expanduser().resolve()
    new = Path(__file__).resolve().parents[1]
    if not old.exists():
        raise SystemExit(f"V2.4.3 folder not found: {old}")
