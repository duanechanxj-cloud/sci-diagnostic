#!/usr/bin/env python3
"""Copy reusable local setup from a V2.4.2 project into V2.4.3."""
...
def main() -> None:
    ...
    parser.add_argument("v242_folder", help="Path to the existing Science Diagnostic System V2.4.2 folder")
    args = parser.parse_args()

    old = Path(args.v242_folder).expanduser().resolve()
    new = Path(__file__).resolve().parents[1]
    if not old.exists():
        raise SystemExit(f"V2.4.2 folder not found: {old}")
